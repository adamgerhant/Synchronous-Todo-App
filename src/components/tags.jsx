...
const Tags = () =>{
    ...
    if(dataContext.tags.length>0){
        return(
            <div className='flex flex-col items-center my-10 rounded-r-lg overflow-hidden border-y border-r border-gray-300 h-[800px] w-[600px]'>
                ...
                <div className='flex flex-col items-center justify-center w-full border-b'>
                   ...
                   <div className="flex flex-row flex-wrap px-10 py-4  min-h-[50px]">
                    {dataContext.tags.map(tag=>(
                        <p onClick={()=>setCurrentTag(tag)} className={`${currentTag&&tag.id==currentTag.id?"bg-blue-300 text-black border border-blue-900":"bg-blue-200 text-blue-900 border border-blue-500  "} cursor-pointer relative flex flex-row items-center pb-[5px] pt-1 pr-12 mr-3 mb-1 font-semibold  text-xl rounded px-2 py-[1px]`}>
                            {tag.name}
